...
One executable, TOML-controlled role configuration (see `fork_config.example.toml` and `docs/FORK_PROFILE_SPEC.md`). No separate Local/Remote executables, no new transport, no auth/voice-call changes. Full rationale in `docs/ADR-0003-DIRECT-IP-ENFORCEMENT.md`.
